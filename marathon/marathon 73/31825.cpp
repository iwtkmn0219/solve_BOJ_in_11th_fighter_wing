...
int find_alphabet_pair(int start, int end) {
	char prev = s[start];
	int result = 1;
...
void change_alphabet(int start, int end) {
...
int main() {
	...
	cin >> n >> q >> s;
	for (int i = 0; i < q; i++) {
		int order, l, r; cin >> order >> l >> r;
		if (order == 1) {
			cout << find_alphabet_pair(l - 1, r - 1) << endl;
		} else {
			change_alphabet(l - 1, r - 1);
		}
	}
}
