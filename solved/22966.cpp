...
struct problem {
	...
	bool operator<(const problem &p) {
		if (difficult != p.difficult) return difficult < p.difficult;
		else return name < p.name;
	}
};

int n;
vector<problem> v_problem;
...
int main() {
	...
	while (n--) {
		...
		int dif;
		cin >> name >> dif;
		v_problem.PB({name, dif});
	}
	sort(v_problem.begin(), v_problem.end());
	cout << v_problem[0].name;
}
