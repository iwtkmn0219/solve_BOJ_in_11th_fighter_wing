...
int main() {
	...
	for (int d = 1; d < n; d++) {
		for (int i = 0; i < n - d; i++) {
			int j = i + d;
			if (d == 1) {
				dp[i][j] = matrix[i].F * matrix[i].S * matrix[j].S;
			} else {
				dp[i][j] = dp[i][i] + dp[i + 1][j] + matrix[i].F * matrix[i].S * matrix[j].S;
				for (int k = 1; k < d; k++) {
					dp[i][j] = min(dp[i][j], dp[i][i + k] + dp[i + k + 1][j] + matrix[i].F * matrix[i + k].S * matrix[j].S);
				}
			}
		}
	}
